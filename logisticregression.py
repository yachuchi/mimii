...
if __name__ == "__main__":
    ...
    # load parameter yaml
    with open("logisticregression.yaml") as stream:
    ...
    for dir_idx, target_dir in enumerate(dirs):
        ...
        eval_labels_pickle = "{pickle}/eval_labels_{machine_type}_{machine_id}_{db}.pickle".format(
                                                                                       ...
                                                                                       machine_id=machine_id,
                                                                                       db=db)
        model_file = "{model}/supervisedmodel_logisticregression_{machine_type}_{machine_id}_{db}.pth".format(model=param["model_directory"],
                                                                                  machine_type=machine_type,
                                                                                  machine_id=machine_id,
                                                                                  db=db)
        ...
        # training
        lg  = LogisticRegression(penalty='l2', dual=False, tol=0.0001, C=1.0, 
                                 fit_intercept=True, intercept_scaling=1, class_weight=None, 
                                 random_state=0, solver='lbfgs', max_iter=100, 
                                 multi_class='auto', verbose=0, warm_start=False, 
                                 n_jobs=None, l1_ratio=None)
        history = lg.fit(train_data, train_data_label)
        torch.save(lg,model_file)
        ...
        for num, file_name in tqdm(enumerate(eval_files), total=len(eval_files)):
            try:
                ...
                y_pred[num] = numpy.round(numpy.mean(lg.predict(data)))
            except Exception as e:
                 print(e)
                 logger.warning("File broken!!: {}".format(file_name))

        train_accuracy = lg.score(train_data, train_data_label)
