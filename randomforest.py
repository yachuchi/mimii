...
if __name__ == "__main__":
    ...
    # load parameter yaml
    with open("randomforest.yaml") as stream:
    ...
    for dir_idx, target_dir in enumerate(dirs):
        ...
        train_labels_pickle = "{pickle}/train_labels_{machine_type}_{machine_id}_{db}.pickle".format(pickle=param["pickle_directory"],
                                                                                       machine_type=machine_type,
        ...
        eval_labels_pickle = "{pickle}/eval_labels_{machine_type}_{machine_id}_{db}.pickle".format(
                                                                                       ...
                                                                                       machine_id=machine_id,
                                                                                       db=db)
        model_file = "{model}/supervisedmodel_randomforest_{machine_type}_{machine_id}_{db}.pth".format(model=param["model_directory"],
                                                                                  machine_type=machine_type,
                                                                                  machine_id=machine_id,
                                                                                  db=db)
        ...
        if os.path.exists(model_file):
            rf.load_weights(model_file)
        else:
            history = rf.fit(train_data, train_data_label)
        torch.save(rf,model_file)
        ...
        for num, file_name in tqdm(enumerate(eval_files), total=len(eval_files)):
            try:
                ...
                y_pred[num] = numpy.round(numpy.mean(rf.predict(data)))
            except Exception as e:
                 print(e)
                 logger.warning("File broken!!: {}".format(file_name))

        train_accuracy = rf.score(train_data,train_data_label)
